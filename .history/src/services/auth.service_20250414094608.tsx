...
class AuthService extends HttpService{
    ...
    getRequest=async(url:string,config:any={})=>{
        try {
            ...
            throw {
                response:exception?.response?.data || null,
    ...
    deleteRequest=async(url:string,data:any,config:any={})=>{
        try {
            this.#setConfig(config);
            let {data:responseData,status} = await axiosInstance.delete(url, { ...this.#config, data });
            return {
                result:responseData,
                status:status
            };

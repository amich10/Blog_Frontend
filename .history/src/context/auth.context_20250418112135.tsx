...
// Define interfaces for credentials, children props, and user details
...
export const AuthProvider = ({ children }: IChildrenProps) => {
  const [userDetails, setUserDetails] = useState<IUserDetails | undefined>(undefined); // Can be undefined initially
  ...
  const LoginFunc = async (credentials: ICredentials) => {
    ...
    try {
      ...
      await getLoggedInUser(); // Get user details after login
    } catch (exception: any) {
  ...
  return (
    <AuthContext.Provider value={{ login: LoginFunc, forgetPassword: forgetPasswordFunc, userDetails:userDetails as IUserDetails, setUserDetails:setUserDetails }}>
...
export const useAuth = () => {
  ...
  return { login, forgetPassword, userDetails, setUserDetails };
};

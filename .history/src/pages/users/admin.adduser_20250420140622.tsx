...
const CreateUser = () => {
  ...
  const sumithandler = async (data: any) => {
      try {
          console.log("Form Data Submitted:", data); // Added clearer log message
          await userSvc.postRequest('/user/create', data);
          notifcation('New user created', NotificationType.SUCCESS);
      } catch (exception) {
          console.error("Error creating user:", exception); // Added error logging
          ...
          <div>
            <div className="flex flex-col gap-1 sm:flex-row sm:gap-4 sm:mb-2">
              <InputLabel htmlFor="fullName" classses={" w-full sm:w-1/3"}>
                ...
                <InputLabel/>
              <TextInputController
                classes={"w-full sm:w-2/3"} // Fixed typo in class name
                name="fullName"
                control={control}
            ...
            <div className="flex flex-col gap-1 sm:flex-row sm:gap-4 sm:mb-2">
              <InputLabel htmlFor="username" classses={" w-full sm:w-1/3"}>Em</InputLabel>
              <TextInputController
                classes={"w-full sm:w-2/3"} // Fixed typo in class name
                name="email"
                control={control}
                errorMsg={errors?.email?.message} // Fixed incorrect error field

...
const CreateUser = () => {
  ...
  const sumithandler = async(data: any) => {
    try {
        ...
        notification('New user created', NotificationType.SUCCESS)
    } catch (exception) {
        notification("User cannot be created now", NotificationType.ERROR)
    }
  };
        ...
        <form onSubmit={handleSubmit(sumithandler)}>
          ...
          <div>
            <div className="flex flex-col gap-1 sm:flex-row sm:gap-4 sm:mb-2">
              <InputLabel htmlFor="fullName" classes={"w-full sm:w-1/3"}>
                Full Name
              </InputLabel>
              <TextInputController

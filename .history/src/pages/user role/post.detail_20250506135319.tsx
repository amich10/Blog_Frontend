...
import { CommentProvider, useComments } from "../../context/comment.context";  // Make sure to import CommentProvider
...
const PostDetail = () => {
  const { slug } = useParams<{ slug: string | undefine0}>();
  const [post, setPost] = useState<BlogPost | null>(null);
  const [isFollowing, setIsFollowing] = useState<boolean>();
  const { totalComments, comments, fetchComments } = useComments();
  ...
  return (
    <CommentProvider slug={slug}> {/* Wrap the component inside CommentProvider */}
    ...
    </CommentProvider> // Wrap the layout here
  );
};

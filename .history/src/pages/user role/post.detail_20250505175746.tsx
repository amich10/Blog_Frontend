...
const PostDetail = () => {
  ...
  onst [likes, setLikes] = useState(post.likes);
  ...
  const handleLike = async () => {
    try {
      ...
      const response = await postSvc.patchRequest(`/post/like/${post.slug}`);
      if (response.data) {
        // Check if the user has already liked the post
        const isLiked = likes.includes(response.data.userId);

        if (isLiked) {
          // Remove the like
          setLikes(likes.filter(like => like !== response.data.userId));
        } else {
          // Add the like
          setLikes([...likes, response.data.userId]);
        }

        // Show success notification
        notification.success({
          message: 'Your like has been updated successfully!',
        });
      }
    } catch (exception) {
      notification.error({
          ...
          <div className="p-6 rounded-lg">
            ...
            <div className="flex items-center gap-4 mb-4">
              <span className="text-white font-bold bg-[#4f6f52] py-1 px-3 rounded-lg">
                ...
                onClick={async () => {
                  try {
                  const response = await postSvc.patchRequest(
                    `/post/like/${post.slug}`,
                    {}, // Provide an empty object or appropriate data as the second argument
                    {} // Provide options if required, or leave as an empty object
                  );
                  if (response.result.data) {
                    if (post.likes.length > 0) {
                    setPost({
                      ...post,
                      likes: post.likes.filter(() => false),
                    });
                    } else {
                    setPost({ ...post, likes: [...post.likes, {}] });
                    }
                    notifcation(
                    "Your like has been updated successfully!",

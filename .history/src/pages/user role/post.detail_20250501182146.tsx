                    ...
                    comments.map((comment: any, idx: number) => (
                      <div key={idx} className="bg-white border border-gray-200 rounded-md p-2">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          />
                          <span className="font-extralight text-md bg-red-300">{comment.userId.username || 'Anonymous'}</span>
                          <span className="text-xs text-gray-400 ml-auto">{comment.createdAt ? new Date(comment.createdAt).toLocaleString() : ''}</span>
                        </div>
                        <div className="text-gray-800 text-sm">{comment.content || comment}</div>
                      </div>
                    ))
                  ) : (

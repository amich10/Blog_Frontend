...
const PostDetail = () => {
  ...
  const getViewsCount = async (data:null) => {
    try {
      // Make the PATCH request and pass an empty object as the second argument
      const response = await postSvc.patchRequest('/post/view/' + slug,data);
  
      // Assuming the response contains data with views and any necessary message
      setViewsCount(response.result.data); // Set the views count
      console.log(response.result.data); // Log the data received from the API
    } catch (exception) {
      console.error('Error fetching view count:', exception); // Log any errors
      throw exception; // Re-throw the exception if you need to handle it elsewhere
    }
  };

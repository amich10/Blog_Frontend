...
const PostDetail = () => {
  ...
  const toggleFollow = async() =>{
    try {
      const apiEndPoint = isFollowing ? `/user/${authorId}/follow` : `/user/${authorId}/unfollow`
      await userSvc.postRequest(apiEndPoint,{})

      setIsFollowing(!isFollowing)
      notifcation("Action successful", NotificationType.INFO);
    } catch (exception) {
      notifcation("Action Failed",NotificationType.ERROR)
    }
  }
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              onClick={async () => {
                try {
                  ...
                  notifcation("You have followd the user",NotificationType.)
                } catch (err) {

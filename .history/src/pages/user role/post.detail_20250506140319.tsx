...
const PostDetail = () => {
  ...
  // Fetch post and comments whenever slug changes
  useEffect(() => {
    if (slug) {
      fetchPost();
      if (slug) fetchComments(slug); // Ensure slug is passed here to fetch comments for that specific post
    }
  }, [slug]);

  // Fetch views count once on component mount
  useEffect(() => {
    if (slug) {
      getViewsCount();
    }

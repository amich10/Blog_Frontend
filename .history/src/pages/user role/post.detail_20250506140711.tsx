...
const PostDetail = () => {
  ...
  const { totalComments, comments, fetchComments, loading: commentsLoading } = useComments();
  ...
  const createComment = async (data: { content: string }) => {
    try {
      if (!post) return;
      await commentSVc.postRequest("/comment/create", { ...data, postId: post._id });
  ...
  const getViewsCount = async () => {
    try {
      ...
      setViewsCount(response.result.data?.views || 0);
    } catch (exception) {
            ...
            <div className="mt-4">
              <form onSubmit={handleSubmit(createComment)}>
                ...
                <TextAreaController
                  ...
                  name="content"
                  rulw={{ required: "Comment content is required" }}
                />
                <Button
                  htmlType="submit"
                  type="primary"
                  className="bg-[#4f6f52] flex justify-center items-center mt-3 w-full"
                >
                  Submit
                </Button>
              </form>

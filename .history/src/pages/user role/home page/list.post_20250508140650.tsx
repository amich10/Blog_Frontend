...
const ListPosts: React.FC = () => {
  ...
  dataSource={posts}
  renderItem={(item) => {
    const author = users.find(user => user._id === item.authorId?._id);
    return (
      <List.Item
        key={item._id}
        actions={[
          <IconText icon={LikeOutlined} text={item.likesCount.toString()} key="like" />,
          <IconText icon={MessageOutlined} text={item.commentsCount.toString()} key="message" />,
        ]}
        extra={
          <img
            width={272}
      ...
      >
        <List.Item.Meta
          avatar={<Avatar src={author?.image?.optimizedUrl} />}
          title={
            <div className="flex flex-col">
              <a
                onClick={() => navigate(`/blogs/${item.slug}`)}
                className="text-[#4f6f52] hover:underline cursor-pointer text-lg font-serif font-semibold"
              >
                {item.title}
              </a>
              <span className="text-gray-500 text-sm mt-1">by {author?.username || 'Unknown Author'}</span>
            </div>
          }
          description={<div className="text-gray-700">{item.excerpt}</div>}
        />
        <div className="mt-3">
          <Button
            type="primary"
            className="!bg-[#4f6f52] hover:!bg-[#3d5842] text-white text-sm font-medium px-5"
            onClick={() => navigate(`/blogs/${item.slug}`)}
          >
            Read More
          </Button>
...
/>
        ...
        <Button
            ...
            >
             ...
             <span className="text-white! font-semibold text-lg"> <FormOutlined/> Write your own Blog</span>
          ...
          {/* Featured Posts */}
          <div className="mt-2">
            <h2 className="text-lg font-bold  text-[#4f6f52]">
              Featured Posts
            </h2>
            <List
            ...
            {/* Who to Follow */}
            <Card
            title={<span className="text-[#4f6f52] font-serif font-bold">Who to Follow</span>}
            bordered={true}

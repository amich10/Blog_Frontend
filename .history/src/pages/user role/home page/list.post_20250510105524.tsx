    ...
    <Content className="shadow-xl rounded-md bg-white relative">
      <div className="flex h-full">
        ...
        <div className="w-1/4 h-full p-4 space-y-8 sticky top-0">
          ...
          >
            <FormOutlined className="mr-2" /> Create a blog
          </Button>

          <div className="border-t border-gray-300 pt-6 text-center">
            <h2 className="font-semibold text-gray-600 mb-2">{randomIdea}</h2>
          </div>

          <div className="space-y-4">
            {users.map((user) => (
              <div
                className="flex gap-2 items-center justify-between border-t pt-4"
                key={user._id}
              >
                <div className="flex items-center gap-2">
                  <Avatar src={user.image?.optimizedUrl} />

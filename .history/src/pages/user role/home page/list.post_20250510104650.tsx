...
const ListPosts: React.FC = () => {
 ...
 const [followedUsers, setFollowedUsers] = useState<Set<string>>(new Set());
  ...
  onClick={async () => {
    try {
      await userSvc.postRequest(`/user/${user._id}/follow`, {});
      setFollowedUsers((prev) => {
        const updated = new Set(prev);
        if (updated.has(user._id)) {
          updated.delete(user._id); // unfollow
          notifcation("You have unfollowed the user", NotificationType.INFO);
        } else {
          updated.add(user._id); // follow
          notifcation("You have followed the user", NotificationType.INFO);
        }
        return updated;
      });
    } catch (exception) {

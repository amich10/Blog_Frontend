  ...
  return (
    <Content className="shadow-xl rounded-md bg-white">
      <div className="flex h-full relative">
          ...
          <div className="flex items-center gap-3 p-6 border-b border-gray-100">
            ...
            >
              {data.map((category, index) => (
                <div
                  key={index}
                  className="flex-shrink-0 bg-gray-100 hover:bg-[#4f6f52] hover:text-white px-4 py-1 rounded-full cursor-pointer transition duration-300 whitespace-nowrap text-sm sticky top-0"
                >
                  {category.title}
                </div>
              ))}
          ...
          <List
            ...
            renderItem={(item) => (
              <List.Item
                key={item._id}
                actions={[
                  <IconText icon={LikeOutlined} text={item.commentsCount.toString()} key="like" />,
              ...
              >
                <List.Item.Meta
                  ...
                  title={<a href={item.title} className="text-[#4f6f52] hover:underline">{item.title}</a>}
        ...
        {/* Right Sidebar */}
        <div className="w-1/4 h-full p-6 space-y-8 sticky top-0 z-10 bg-pink-400">
          {/* Featured Posts */}
            <div>

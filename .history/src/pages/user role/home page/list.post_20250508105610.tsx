...
const ListPosts: React.FC = () => {
  ...
  const [totalComment, setTotalComments] = useState<Record<string, number>>({});

  const retrieveTotalComments = async (slugs: string[]) => {
    try {
      const responses = await Promise.all(
        slugs.map((slug) =>
          commentSVc.getRequest('/comment/post/count/' + slug)
        )
      );
      const commentsCountMap = responses.reduce((acc, response, index) => {
        acc[slugs[index]] = response.result.data.totalComments;
        return acc;
      }, {} as Record<string, number>);
      setTotalComments(commentsCountMap);
    } catch (exception) {
  ...
  useEffect(() => {
    const fetchPostsAndComments = async () => {
      ...
      if (posts.length > 0) {
        ...
        await retrieveTotalComments(slugs);
      }
    };

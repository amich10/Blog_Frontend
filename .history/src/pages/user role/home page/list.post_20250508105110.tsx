...
const ListPosts: React.FC = () => {
  ...
  const retrieveTotalComments = async (slug: string) => {
    try {
      const response = await commentSVc.getRequest('/comment/post/count/' + slug);
      return response.result.data.totalComments;
    } catch (exception) {
      ...
      try {
        ...
        if (posts.length > 0) {
          const totalComments = await Promise.all(
            posts.map((post) => retrieveTotalComments(post.slug))
          );
          setTotalComment(totalComments.reduce((acc, count) => acc + count, 0));
        }
      } catch (error) {
        console.error(error);
      }

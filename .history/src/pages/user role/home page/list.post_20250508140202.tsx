...
const ListPosts: React.FC = () => {
  ...
  return (
   <Content className="min-h-screen shadow-xl rounded-md bg-white relative">
  <div className="flex h-[calc(100vh-64px)] overflow-hidden">
    {/* Left Panel - Scrollable */}
    <div className="w-3/4 border-r border-gray-200 h-full px-6 overflow-y-auto">
      {/* Category Scrollbar */}
          ...
          >
            <List.Item.Meta
              avatar={
                <Avatar
                  src={
                    users.find((user) => user._id === item.authorId?._id)?.image?.optimizedUrl
                  }
                />
              }
              title={
    ...
    {/* Right Sidebar - Fixed */}
    <div className="w-1/4 h-full p-6 space-y-8 sticky top-0 bg-white">
      <Button
        type="primary"
      ...
      {/* Featured Posts */}
      <div className="mt-2">
        <h2 className="text-lg font-bold text-[#4f6f52]">Featured Posts</h2>
        <List
          ...
          renderItem={(user) => (
            ...
            >
              <List.Item.Meta
                ...
                title={<span className="text-[#4f6f52]">{user?.username}</span>}
              />
            </List.Item>
          )}
        />

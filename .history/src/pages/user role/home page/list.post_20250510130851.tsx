      ...
      <div className="flex h-full">
        ...
        <div className="w-3/4 border-r border-gray-200 h-full px-6">
          ...
          <div className="flex items-center gap-3 p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
            <Button
              icon={<LeftOutlined />}
              className="!text-violet-600 !border-none"
              onClick={() => scroll(-150)}
            />
            <div
              ref={sliderRef}
            ...
            >
              {[{ _id: "all", title: "All Posts" }, ...data].map(
                (category) => (
                  <div
                    key={category._id}
                    onClick={() => {
                    ...
                    className={`flex-shrink-0 px-4 py-1 rounded-full cursor-pointer transition duration-300 whitespace-nowrap text-sm ${
                      ...
                      (category._id === "all" ? null : category._id)
                        ...
                        : "bg-gray-100 hover:bg-violet-600 hover:text-white"
                    }`}
                  >
                    {category.title}
                  </div>
            ...
            renderItem={(item) => (
              <List.Item
                ...
                actions={[
                  ...
                  <Button
                    type="link"
                    className="text-violet-600! hover:bg-green-900"
                    icon={
                      <IconText
                ...
                <List.Item.Meta
                  ...
                  title={
                    <div className="flex flex-col">
                      ...
                      >
                        <span className="text-violet-600 font-serif font-semibold">
                          {" "}
                          {item.title}
                        </span>
                      </a>
                  ...
                  description={
                    <div>
                      ...
                      <Button
                        type="link"
                        className="text-violet-600! font-semibold! font-serif! text-xs! underline!"
                        onClick={() => navigate(`/blogs/${item.slug}`)}
                      >
                        Load More
                      </Button>
        ...
        <div className="w-1/4 h-full p-4 space-y-8 sticky top-0">
          <Button
            type="primary"
            className="!bg-violet-600 hover:!bg-[#3d5842] w-full h-20! flex justify-center items-center mb-3! shadow-lg! border-b-2! border-gray-300!"
          ...
          <Card
            className="mt-2"
            title={
              <span className="text-violet-600 font-semibold font-serif text-xl">
                Featured Posts
              </span>
            }
          >
                ...
                <List.Item>
                  <List.Item.Meta
                    title={
                      <span
                        onClick={() => navigate(`/blogs/${item.slug}`)}
                        className="text-violet-600 font-semibold font-serif hover:cursor-pointer"
                      >
                        {item.title}
                      </span>
                    }
          ...
          {/* Who to Follow */}
            <Card
            title={
                <span className="text-violet-600 font-serif font-bold">
                  Follow others
                </span>
            }
            bordered={true}

const UserHeader = () => {
  const { userDetails, setUserDetails } = useAuth();
  const navigate = useNavigate();

  const handleLogoClick = () => {
    // Navigate to the main blogs page
    navigate("/blogs");
    // Reset search and selected category state
    setSearch(""); // Reset search
    setSelectedCategoryId(null); // Reset selected category
    setPagination((prev) => ({ ...prev, current: 1 })); // Reset pagination
    retrievePosts({
      page: 1,
      limit: pagination.pageSize,
      search: "",
      categoryId: null,
    });
  };

  const menuItems: MenuProps["items"] = [
    {
      key: 1,
      label: <NavLink to={"/blogs/profile"}><UserOutlined /> Profile</NavLink>,
    },
    {
      key: 2,
      label: (
        <NavLink
          to={"/"}
          onClick={(event) => {
            event.preventDefault();
            removeLocalStorage(webStorageConstants.ACCESS);
            removeLocalStorage(webStorageConstants.REFRESH);
            setUserDetails(undefined);
            navigate("/");
          }}
        >
          <LogoutOutlined /> Signout
        </NavLink>
      ),
    },
  ];

  return (
    <Layout.Header className="bg-white! flex border-b border-gray-100 m-0 rounded-md ">
      <div className="flex items-center justify-between w-full">
        <div className="flex justify-center items-center gap-5">
          <div className="font-serif text-2xl font-bold" onClick={handleLogoClick}>
            <span className="text-[#4f6f52]!">Writelle</span>
          </div>
        </div>
        <div className="flex">
          <Dropdown menu={{ items: menuItems }} placement="bottomCenter" trigger={['click']}>
            <Avatar
              src={userDetails?.image?.optimizedUrl}
              alt="user"
              className="h-15 w-15 rounded-full cursor-pointer"
            />
          </Dropdown>
        </div>
      </div>
    </Layout.Header>
  );
};

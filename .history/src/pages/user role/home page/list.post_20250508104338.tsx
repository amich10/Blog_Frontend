...
const ListPosts: React.FC = () => {
  ...
  const retrieveTotalComments = async () => {
    try {
      ...
      const totalComments = response.map((res) => res.result.data.totalComments);
      setTotalComment(totalComments.reduce((acc, curr) => acc + curr, 0));
  ...
  useEffect(() => {
    ...
    if (posts.length > 0) {
      retrieveTotalComments(posts[0].slug);
    }
  }, []);

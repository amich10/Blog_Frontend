...
const ListPosts: React.FC = () => {
  ...
  const loggedInuserId = userDetails._id;
   ...
   const handleFollowClick = async (authorId: string) => {
    try {
      // Fetch current follow status for the user
      const res = await userSvc.getRequest(`/user/${authorId}/follow-status`);
      const followStatus = res.result.data.isFollowing;
      setIsFollowing(followStatus); // Set the initial state
      ...
      notifcation(
        isFollowing ? "You have unfollowed the user" : "You have followed the user",
        ...
        <div className="w-1/4 h-full p-4 space-y-8 sticky top-0">
          ...
          >
            <List
              ...
              dataSource={
                posts
                  .filter((post) => post.authorId && loggedInuserId && post.authorId._id !== loggedInuserId)
                ...
                <List.Item
                  actions={[
                    <Button
                      ...
                      key="follow"
                      onClick={()=>
                         handleFollowClick(post.authorId._id)}
                    >
                ...
                >
                  <List.Item.Meta
                    avatar={
                      <Avatar
                        src={
                          users.find((user) => user._id === post.authorId._id)
                            ?.image?.optimizedUrl
                        }
                      />
                    }
                    title={
                      <span className="text-[#4f6f52]">{post.authorId?.username}</span>
                    }
                  />
                </List.Item>
              )}

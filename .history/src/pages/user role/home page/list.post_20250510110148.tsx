...
const ListPosts: React.FC = () => {
  ...
  // Get the authorId of the first post (if any) to check follow status
  const firstAuthorId = posts.length > 0 ? posts[0].authorId._id : null;

  useEffect(() => {
    const fetchFollowStatus = async () => {
      try {
        if (!firstAuthorId) return;
        const res = await userSvc.getRequest(`/user/${firstAuthorId}/follow-status`);
        console.log(res);
    ...
    if (firstAuthorId) fetchFollowStatus();
  }, [firstAuthorId]);
                ...
                <List.Item
                  actions={[
                    <Button
                      ...
                      onClick={async() => {
                        try {
                          await userSvc.postRequest(`/user/${firstAuthorId}/follow`,{})

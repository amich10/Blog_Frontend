...
useEffect(() => {
  ...
  // If we're on the blogs root path, reset filters
        ...
        <div className="w-3/4 border-r border-gray-200 h-full px-6">
          ...
          <List
            ...
            renderItem={(item) => (
                <List.Item
                key={item._id}
                className="border border-gray-100 rounded-2xl gap-2 mb-4 shadow"
                actions={[
                  <Button
        ...
        <div className="w-1/4 h-full p-4 space-y-8 sticky top-0">
          ...
          >
            ...
            <Card className="mt-2" title = {
              <span className="text-[#df6f52] font-semibold font-serif">
                Fetaured Posts
              </span>
            }>
            <List className="border border-gray-100 px-3!"
              ...
              <List.Item>
                <List.Item.Meta
                title={
                  <span onClick={() => navigate(`/blogs/${item.slug}`)} className="text-[#4f6f52] font-semibold font-serif hover:cursor-pointer">
                  {item.title}
                  </span>
                }
                description={
                  <span className="text-[14px]">
                  {item.excerpt.length > 100 ? `${item.excerpt.slice(0, 80)}...` : item.excerpt}
                  <Button
                    type="link"
                    className="text-[#4f6f52]! font-semibold! font-serif! text-xs! underline!"
                    onClick={() => navigate(`/blogs/${item.slug}`)}

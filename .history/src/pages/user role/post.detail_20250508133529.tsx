...
const PostDetail = () => {
  ...
  const [comments, setComments] = useState<
    Array<{
      postId: string;
      userId: { image?: { optimizedUrl: string }; username?: string };
      createdAt?: string;
      content: string;
    }>
  >([]);
  ...
  const [liked, setLiked] = useState<boolean>(false); // Track if the post is liked
  ...
  /* const likePost = async () => {
    try {
      const response = await postSvc.patchRequest(`/post/like/${slug}`, {});
      ...
      notifcation("Post liked successfully!", NotificationType.SUCCESS);
    } catch (exception) {
      console.log(exception)
      notifcation("Failed to like post.", NotificationType.ERROR);
    }
  }; */
 /*  const unlikePost = async () => {
    try {
      const response = await postSvc.patchRequest(`/post/unlike/${slug}`, {});
      ...
      notifcation("Post unliked successfully!", NotificationType.SUCCESS);
    } catch (exception) {
      console.log(exception);
      notifcation("Failed to unlike post.", NotificationType.ERROR);
    }
  }; */
  ...
  const getViewsCount = async () => {
    try {
      ...
      console.log(response.result.data?.views);
    } catch (exception) {

...
import { IQueryType } from "../../../interfaces/interfaces";
...
const UserList = () => {
  ...
  const getAllUsers = async ({
    ...
    try {
      setLoading(true);
      const response: IRe = await userSvc.getRequest("/user/all", {
        params: {
          limit: limit,
          page: page,
          search: search,
      ...
      notifcation(
        "Sorry user list cannot be generated now.",
    ...
    },
    {
      ...
      render: (id: string) => (
        <div className="flex gap-5">
          <NavLink to={"/admin/users/" + id} className="text-green-500!">
            <EditOutlined />
          </NavLink>
          <Popconfirm
            title="Arey your sure"
            ...
            onConfirm={async () => {
              try {
                ...
                notifcation(
                  "Sorry, the selected user cannot be deleted now.",
                  NotificationType.ERROR
                );
              }
            }}

...
import { useDispatch, useSelector } from "react-redux";// Redux logout action
import { RootState } from "../../store/store"; // RootState for useSelector
...
const AdminHeader = ({ collapsed, setCollapsed }: ICollapseProps) => {
  ...
  const userDetails = useSelector((state: RootState) => state.auth.userDetails); // Accessing user details from Redux store
      ...
      label: (
        <NavLink
          ...
          onClick={(event:any) => {
            ...
            dispatch(lo()); // Dispatch Redux action to logout

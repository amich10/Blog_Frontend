...
const ResetPasswordPage = () => {
  ...
  const verifyToken = async() =>{
    try {
        const token = params.forgetToken
        const response = await authSvc.getRequest('auth/verify-token/'+token)
        setToken(response.result.data.verifyToken)
        setValue('token',response.result.data.verifyToken)
    } catch (exception) {
        console.log(exception)
        notifcation("Sorry token cannot be verified at this time",NotificationType.ERROR)
        navigate('/forget-password')
    }
  }

  const resetPasswordhandler =async(data:{token:string,password:string,confirmPassword:string}) =>{
    try {
        const response = await authSvc.patchRequest('password-reset',data)
        notifcation(response.result.data.verifyToken,NotificationType.SUCCESS)
        navigate('/')
    } catch (exception:any) {
        console.log(exception)
        notifcation(exception.)
    }
  }

    ...
    <div className="h-full flex justify-center items-center mt-15">
      <div className="w-2/4 border-2 border-green-700 font-serif p-4 rounded-md">
        ...
        >
          ...
          <div className="mt-3">
        <InputLabel htmlFor="socialLinks" className="block">
          Social Media (URL)
        </InputLabel>

        {/* Render the socialLinks dynamically */}
        <div>
          {control.getValues("socialLinks").map((_, index) => (
            <div key={index}>
              <Controller
                ...
                render={({ field }) => (
                  <TextAreaController
                    {...field}
                    placeholder={`Enter your social link ${index + 1}`}
                    errorMsg={errors?.socialLinks?.[index]?.message}
                  />
                )}
              />
              {/* Option to add more links */}
              {index === control.getValues("socialLinks").length - 1 && (
                <button
                  type="button"
                  onClick={() => control.setValue("socialLinks", [...control.getValues("socialLinks"), ""])}
                >
                  Add another link
                </button>
              )}

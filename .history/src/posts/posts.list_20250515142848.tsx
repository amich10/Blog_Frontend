...
import { Input, Layout, Popconfirm, Table, Typography } from "antd";
...
const PostList = () => {
  const tableColumns = [
    ...
    },
    {
      ...
      render: (category: IPostType["categoryId"]) =>
        category ? (
          <span className="bg-yellow-100! px-2 py-1 rounded-md text-xs text-yellow-700!">
            {category.title}
          </span>
        ) : (
    ...
    },
    {
      ...
      render: (content: string) => (
        <span className="text-xs">{content.slice(0, 50) + "..."}</span>
      ),
    },
    {
      title: "Tags",
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              try {
                ...
                notifcation(
                  "Sorry, the post cannot be deleted now. Please try again later.",
  ...
  const { posts, setSearch, getallPosts, pagination, search, loading } =
    usePost();

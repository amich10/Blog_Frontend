...
const PostList = () => {
    ...
    const listAllPosts = async({page=pagination.current,limit=pagination.current,search=""}:IQueryProps) =>{
        try {
            ...
            setPagination((prev) => ({
                ...prev,
                pageSize: posts?.result?.options?.pagination?.limit || prev.pageSize,
                total: posts?.result?.options?.pagination?.total || prev.total,
                current: posts?.result?.options?.pagination?.page || prev.current,
            }));
        } catch (exception) {

...
const CreatePost = () => {
    ...
    const CreatePostSchema = Yup.object({
        title: Yup.string().min(2, "Full Name must be at least 2 characters").max(50, "Full Name cannot exceed 50 characters").required("Full Name is required"),
        postname: Yup.string().min(2, "postname must be at least 2 characters").max(50, "postname cannot exceed 50 characters").required("postname is required"),
      ...
      <Layout.Content className="bg-white p-8 rounded-md shadow-md">
        <form onSubmit={handleSubmit(sumithandler)}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-1 sm:flex-row sm:gap-4 sm:mb-2">
              <InputLabel htmlFor="postname" classses={" w-full sm:w-1/3"}>
                postname
              </InputLabel>
              <TextInputController
                classes={"w -full sm:w-2/3"}
                name="postname"
                control={control}
                errorMsg={errors?.postname?.message}

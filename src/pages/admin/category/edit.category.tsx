...
const EditCategory = () => {
  ...
  const getCategoryById = async () => {
    try {
      ...
      notifcation(
        "Sorry category cannot be fetched now.",
  ...
  const categoryUpdateById = async (data: any) => {
    try {
      await categorySVc.patchRequest("/category/update/" + params.id, data, {
        file: true,
      });
      ...
      notifcation(
        "Sorry, category cannot be updated now.Please try again later",
        NotificationType.ERROR
      );
    }
  };
  ...
  return (
    <>
      <Layout.Content className="p-8 bg-white rounder-md shadow-xl ">
        <form
          onSubmit={handleSubmit(categoryUpdateById)}
          onReset={() => {
            reset();
